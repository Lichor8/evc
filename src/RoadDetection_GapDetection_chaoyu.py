...
sumcanny=np.sum(imgCanny)
#print("canny value\n",imgCanny,"sumcanny",sumcanny)
cannyshape=imgCanny.shape
#print("canny shape\n",cannyshape)
cannyimgsize_x=cannyshape[0]
cannyimgsize_y=cannyshape[1]
cannyimgsize=cannyimgsize_x*cannyimgsize_y
length=len(imgCanny)
#print("canny length\n",length)
sumofcanny=imgCanny.sum()
#print("canny sum\n",imgCanny[0])
#print("imgsize \n",cannyimgsize)
...
#             total = total + input[row][col]
#             return total
# a=[[1, 2], [3, 4], [5, 6]]
# def sum1(input):
#     return sum(map(sum, input))
# total= sum(map(sum, imgCanny))
# print("sum1 \n",total)
# cannyshapea=a.shape
# print("ashape",cannyshapea)
# cannyimgsize_ax=a[0]
# cannyimgsize_ay=a[1]
# total3=0
# for i in range(0,3):
#     for j in range(0,2):
#         total3=total3+a[i][j]
# print("totalvalue imgcanny3 \n",total3)
#
# total2 = 0
# for i in range(0,cannyimgsize_x):
#     for j in range(0,cannyimgsize_y):
#         if imgCanny[i][j]==255:
#             total2=total2+1
...
##roiGap.x = int(0.5*(imgSize.x - roiSize.x*imgSize.x/100))
...
roirangex=50
roirangey=50
roi = imgCanny[receivedpos_x-roirangex:receivedpos_x+roirangex,receivedpos_y-roirangey:receivedpos_y+roirangey ]       # get roi from image
imgRoi = np.zeros((imgSize.y, imgSize.x), np.uint8)                 # create new binary black image
imgRoi[receivedpos_x-roirangex:receivedpos_x+roirangex,receivedpos_y-roirangey:receivedpos_y+roirangey] = roi     # add roi to black image

roiShape = roi.shape
roiSize_x = roiShape[0]
roiSize_y = roiShape[1]
print("roishape \n",roiShape,"roix roiy \n", roiSize_x,roiSize_y)

concentration_roi = 0
for i in range(0,roiSize_x):
    for j in range(0,roiSize_y):
        if roi[i][j]==255:
